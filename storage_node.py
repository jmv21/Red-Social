...
class StorageNode:
    def __init__(self, addr, K, index, storage_nodes = None, vocal_option = False):
        ...
        #SEND UPDATED LIST 
        #self.send_updated_node_list(local_requester)

        thr_check_succ = threading.Thread(target = self.wrapper_check_on_succ, args =())
        thr_check_succ.start()
        print('started thread')

        #thr_wait_for_command = threading.Thread(target = self.waiting_for_command, args=[local_requester])
        #thr_wait_for_command.start()
    ...
    def check_on_succ(self, local_requester):
        # JUST FOR NOW
    ...
    def create_new_storage_node(self, local_requester):
